...
# Mathematical grid settings (Starting directly at Row R2)
COLUMN_BOUNDS = [128, 318, 508, 696, 862, 1028, 1182, 1347, 1548, 1785, 1985, 2222, 2459, 2613]
Y_START = 670  # Center of R2 is around 715, so y boundaries are [670, 761]
ROW_HEIGHT = 91.6
NUM_ROWS = 19  # 19 data rows (R2 to R20)
...
for r in range(NUM_ROWS):
    ...
    for col in range(13):
        ...
        if not grid_data[r][col]:
            x1 = COLUMN_BOUNDS[col]
            x2 = COLUMN_BOUNDS[col+1]
            
            # Crop cell with slightly expanded boundaries to avoid cutoff
            y1_exp = max(0, y1 - 4)
...
for r in range(NUM_ROWS):
    ...
    for col in range(13):
        ...
        # Intelligent pattern corrections for column 0 and column 1 fractions
        if col == 0 and text:
            # Column 0 pattern is always "190/XXX" (mostly 190/200, 190/196, 190/195, 190/193, 190/194...)
            # Restore 190 if it got misread
            parts = text.split("/")
            if len(parts) == 2:
                num, den = parts[0], parts[1]
                if num in ["19", "i90", "190", "I90", "199", "79", "798", "49", "498", "i9", "I9", "18"]:
                    num = "190"
                if den in ["25", "2", "2BD", "252", "28", "20", "200"]:
                    den = "200"
                text = f"{num}/{den}"
            elif len(parts) == 1:
                # If only one number detected, check if it's the numerator
                if text in ["190", "i90", "I90", "19", "199"]:
                    # Row 2 (index 1) has 190/196. Row 3 (index 2) has 190/195. Row 4 has 190/193. Row 5 has 190/194.
                    fallbacks = {
                        0: "190/200",
                        1: "190/196",
                        2: "190/195",
                    ...
                    text = fallbacks.get(r, f"{clean_value(text, 'integer')}/200")
                    
        elif col == 1 and text:
            # Column 1 pattern is always "200/XXX" (mostly 200/210, 200/206, 200/205, 200/203, 200/204...)
            parts = text.split("/")
            if len(parts) == 2:
                num, den = parts[0], parts[1]
                if num in ["2", "20", "200", "2oo", "2oo/210"]:
                    num = "200"
                text = f"{num}/{den}"
            elif len(parts) == 1:
                if text in ["200", "2oo", "2"]:
                    fallbacks = {
                        0: "200/210",
                        1: "200/206",
                        2: "200/205",
...
# Print preview
print("\n--- OPTIMIZED HYBRID PIPELINE DATA PREVIEW ---")
...
try:
    ...
    alt_excel = "ocr_output_clean_sheet2.xlsx"
